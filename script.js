...
let locationId =
  "geAOl3NEW1iIKIWheJcj" ?? inventoryPanel.getAttribute("data-locationId");
let setting = {
  inv_detail_path: "nolocodetailpage.html",
  inv_list_path: "nolocoinventry.html",
  detailPath: "nolocodetailpage.html",
  listPath: "nolocoinventry.html",
  dev: "local",
  inv_financed_path: "/finance",
  inv_inquire_path: "/inquire",
};
...
let pathsData = redPath[setting.dev] ?? false;
if (pathsData) {
  setting = { ...setting, ...pathsData };
}
setting.dev = typeof pathsData == "object";
...
function getPath(list = true, def = "") {
...
function detailPageInit() {
  ...
  function displayDetail(item, ignoreCache = false) {
    ...
    <div class="equipment-content" id="equipmentContent">
      ...
      setTimeout(function () {
        document.querySelectorAll(".payment-calculator .field").forEach((t) => {
  ...
  (async () => {
    const response = await fetch(`${url}get/${uuid}?locationId=${locationId}`);
    if (!response.ok) {
...
function listPageInit() {
  ...
  function handleInventoryData(
    ...
    inventoryData = (inventoryCollection?.edges ?? []).map((edge) => edge.node);
  ...
  async function fetchInventory(afterCursor = "", filters = {}, sorting = {}) {
    try {
      toggleLoader(true);
...
function inventorySlider(){
  ...
  function initializeCarousel() {
    const url = "https://gostarauto.com/api/inventory/list";
...
<img src="${vehicle?.node?.photosUrls ?? ''}"  alt="2013 Audi Q5">

        </div>
        <div class="info">
        <div>
            <h2>${ vehicle?.node?.name ?? ""}</h2>
            <p>${ vehicle?.node?.miles ?? ""} mi &bull; ${ vehicle?.node?.listedPrice ?? ""} &bull; 2.0T quattro Premium</p>
            </div>
            <div>
